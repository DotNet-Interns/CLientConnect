...
function NoteCard() {
    return (
        <div className='container w-25 notes rounded-2 shadow-sm d-flex flex-column justify-content-start align-items-center p-2 gap-2 pointer'>
...
function DisplayCustomer() {
    ...
    useEffect(() => {
        const fetchCustomers = async () => {
            try {
                setLoading(true);
                const response = await axios.get("http://172.20.68.11:5100/api/Customers/2", {
                            ...
                            <div className="d-flex flex-column align-items-start justify-content-start">
                                ...
                                <ul className="list-group w-100">
                                            ...
                                            <div className="position-relative">
                                                ...
                                                {activeMenuIndex === index && (
                                                    <div className="dropdown-menu show">
                                                        <button className="dropdown-item" onClick={() => openModal("Phones", "edit", phoneNumber.pid, phoneNumber.phone, index)}>
                                                            <MdOutlineEdit /> Edit
                                                        </button>
                                                        <button className="dropdown-item" onClick={() => openModal("Phones", "delete", phoneNumber.pid, phoneNumber.phone, index)}>
                                                            <MdDeleteOutline /> Delete
                                                        </button>
                                                        <button className="dropdown-item" onClick={() => copyToClipboard(phoneNumber.phone, activeMenuIndex)}>
                                                            <MdCopyAll /> Copy
                                                        </button>
                                                    </div>
                                                )}
                                    ...
                                    <li className="list-group-item p-0 bg-secondary-subtle pointer d-flex justify-content-center align-items-center">
                                        <button className="btn w-100" onClick={() => openModal("Phones", "add", customers.cid)}>
                                            <IoAdd />
                                        </button>
                                    </li>
                                </ul>
                                ...
                                <ul className="list-group w-100">
                                    {customers.emails.map((email, index) => (
                                        ...
                                        >
                                            {email.email}
                                            <div className="position-relative">
                                                <IoOptions className="options-icon" onClick={() => handleOptionsClick(index + customers.phoneNumbers.length)} />
                                                {activeMenuIndex === index + customers.phoneNumbers.length && (
                                                    <div className="dropdown-menu show">
                                                        <button className="dropdown-item" onClick={() => openModal("Emails", "edit", email.eid, email.email, activeMenuIndex)}>
                                                            <MdOutlineEdit /> Edit
                                                        </button>
                                                        <button className="dropdown-item" onClick={() => openModal("Emails", "delete", email.eid, email.email, activeMenuIndex)}>
                                                            <MdDeleteOutline /> Delete
                                                        </button>
                                                        <button className="dropdown-item" onClick={() => copyToClipboard(email.email, activeMenuIndex)}>
                                                            <MdCopyAll /> Copy
                                                        </button>
                                                    </div>
                                                )}
                                    ...
                                    <li className="list-group-item p-0 bg-secondary-subtle pointer d-flex justify-content-center align-items-center">
                                        <button className="btn w-100" onClick={() => openModal("Emails", "add", customers.cid)}>
                                            <IoAdd />
                                        </button>
                                    </li>
                                </ul>
